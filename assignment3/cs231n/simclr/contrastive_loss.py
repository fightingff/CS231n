...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        D = math.exp(1. / tau)
        total_loss += - sim(z_k, z_k_N) / tau * 2
        
        S1 = - torch.exp(sim(z_k, z_k) / tau)
        S2 = - torch.exp(sim(z_k_N, z_k_N) / tau)
        for j in range(2*N):
            S1 += torch.exp(sim(z_k, out[j]) / tau)
            S2 += torch.exp(sim(z_k_N, out[j]) / tau)

        total_loss += torch.log(S1) + torch.log(S2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs /= torch.linalg.norm(out_left, dim = 1).view(-1, 1)
    pos_pairs /= torch.linalg.norm(out_right, dim = 1).view(-1, 1)
...
def compute_sim_matrix(out):
    ...
    Mod = torch.linalg.norm(out, dim = 1).view(-1, 1)
    sim_matrix = (out @ out.T) / (Mod @ Mod.T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = None
    denom = torch.sum(exponential, dim=1).view(-1, 1)
    ...
    pairs = torch.cat([pos_pairs, pos_pairs], dim = 0)
    ...
    numerator = torch.exp(pairs / tau).view(-1, 1) 
